...
func TestTreeApplyBatch(t *testing.T) {
	...
	ft := NewNumeric([]int64{1, 2, 3, 4})
	err := ft.Apply(
		AddMutation(0, int64(5)),
		SetMutation(1, int64(10)),
		AddMutation(1, int64(-3)),
		SetMutation(3, int64(8)),
	)
	if err != nil {
		t.Fatal(err)
	}
	...
	if got := ft.Total(); got != 24 {
		t.Fatalf("Total()=%d, want 24", got)
	}
}

func TestTreeApplyValidatesBeforeMutation(t *testing.T) {
	t.Parallel()

	ft := NewNumeric([]int64{1, 2, 3})
	before := ft.Values()

	err := ft.Apply(
		AddMutation(0, int64(10)),
		AddMutation(3, int64(1)),
	)
	...
	err = ft.Apply(Mutation[int64]{Index: 0, Kind: MutationKind(99), Value: 1})
	if err == nil {
		t.Fatal("Apply invalid kind returned nil")
	}
	if got := ft.Values(); !equalInt64s(got, before) {
		t.Fatalf("invalid kind partially applied: got %v want %v", got, before)
	}
}

func TestTreeApplyEmptyAndNil(t *testing.T) {
	t.Parallel()

	ft := NewNumeric([]int64{1})
